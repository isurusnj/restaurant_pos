...
authorize(['admin', 'manager']);                   // protect page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'move') {
    ...
    $qty    = (int)($_POST['qty'] ?? 0);          // +ve restock, -ve deduct
    $note   = trim($_POST['note'] ?? '');

    if ($itemId <= 0 || $qty === 0) {
        $errors[] = 'Choose an item and a non-zero quantity.';
    } else {
        try {
            $pdo->beginTransaction();

            // Update stock (treat NULL as 0)
            ...
            $ins = $pdo->prepare("
                ...
                VALUES (?, ?, ?, NULL, ?)
            ");
            $ins->execute([$itemId, $qty, $reason, $note ?: null]);
        ...
        <div class="login-field">
            ...
            <select name="menu_item_id" required>
                ...
                <?php foreach ($items as $mi): ?>
                    ...
                    ?>
                    <option value="<?= (int)$mi['id'] ?>"><?= htmlspecialchars($label) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
...
<?php include __DIR__ . '/../layouts/footer.php'; ?> <!-- <-- fixed path -->